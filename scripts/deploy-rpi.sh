...
if [ ! -e "$env_file_location" ]; then
    ...
    docker run -d --restart unless-stopped -e PRODBOTTOKEN=$PRODBOTTOKEN -e NODE_ENV=production -e JSONPATH=$JSONPATH --name $CONTAINER_NAME -v $VOLUME_NAME:$JSONPATH $IMAGE_NAME
    '
fi 
